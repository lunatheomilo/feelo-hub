...
Please recommend 3–5 games for right now.`;
  ...
  try {
    ...
    // If Anthropic returned an error object, surface it clearly instead of
    // silently trying (and failing) to treat it as a recommendation payload.
    if (data.error) {
      console.error("Anthropic API error:", data.error);
      return res.status(500).json({ error: "Failed to generate recommendations", detail: data.error });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: "Failed to generate recommendations", detail: "Could not parse model response as JSON", raw: text });
    }
